...
class StateDescriptor(nn.Module):
    """
    Build routing descriptors from cheap, interpretable local signals.
    Output channels: [roughness, edge, anisotropy, reliability] per spatial location.
    """
...
class PrimitiveOperatorBank(nn.Module):
    """Shared primitive basis transforms executed once per block."""
...
class OperatorAtomMixer(nn.Module):
    """
    Convert a small shared primitive bank into a set of latent operators.
    Operator k is not a standalone branch; it is a learned mixture over primitive bases.
    """
...
class CleanStateEncoder(nn.Module):
    """
    Preserve the clean, proposal-friendly backbone signal.
    Keep this path simple and dominant.
    """
...
class GeometryStateEncoder(nn.Module):
    """
    Lightweight geometry increment:
    - strip orientation
    - diagonal continuity
    - local peak / separation
    """
...
class OrthogonalStateMixer(nn.Module):
    """
    Clean path is the anchor.
    Geometry path only injects the non-redundant increment.
    """
...
class DualStateOrthogonalCell(nn.Module):
    """
    v2-based low-coupling dual-state enhancement.

    Important:
    - not a new heavy backbone
    - not multi-branch takeover
    - only a small early-stage increment
    """
...
class DualPathDownsample(nn.Module):
    """
    Borrow only the useful part from strong detectors:
    keep both conv texture and local peak signal during downsampling.
    """
...
class DORSANet(nn.Module):
    ...
    Design philosophy:
    - keep the efficient v2 trunk as the foundation
    - only strengthen stage1 / stage2
    - clean state remains dominant
    - geometry only contributes orthogonal increment
    """
    def __init__(
        self,
        in_chans=3,
