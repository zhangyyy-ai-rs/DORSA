...
class BaseNet_MobileNetv2(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, pretrained=True):
        super(BaseNet_MobileNetv2, self).__init__()
...
class BaseNet_DORSA_T(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, pretrained=True):
        super(BaseNet_DORSA_T, self).__init__()
        self.backbone = DORSA.DORSANet(
            in_chans=3,
            stem_dim=48,
            depths=(2, 2, 6, 2),
            mlp_ratio=2.0,
        ...
        if pretrained:
            for ckpt_path in (
                './backbone_weights/dorsa_t_2262_s48.pth',
                './backbone_weights/dorsa_t.pth',
            ):
                if os.path.isfile(ckpt_path):
                    checkpoint = torch.load(ckpt_path, map_location=torch.device('cuda:0'))
                    state_dict = checkpoint.get('model', checkpoint.get('state_dict', checkpoint))
                    missing_keys, unexpected_keys = self.backbone.load_state_dict(state_dict, False)
                    print('The weights are successfully loaded\n')
                    print('missing_keys: ', missing_keys)
                    print('unexpected_keys: ', unexpected_keys)
                    break

        channles = [48, 96, 192, 384]
        self.en_d = 48
        self.mid_d = self.en_d * 2
        self.swa = NeighborFeatureAggregation(channles, self.mid_d)
        self.tfm = TemporalFusionModule(self.mid_d, self.en_d * 2)
